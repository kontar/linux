							...
							int alen, int flags)
{
	...
	{
		pr_debug("sk_type Expected %d : got %x\n",SOCK_SEQPACKET, sk->sk_type);
		return -EINVAL;
	}
	...
	if (alen < sizeof(*sa))
	{
		pr_debug("alen Expected < %d : got %x\n",sizeof(*sa), alen);
		return -EINVAL;
	}
	...
	if (!vrp_channels) {
		pr_debug("Error tree lookup:rpmsg_channels %p : vproc_id %d\n",&rpmsg_channels, sa->vproc_id);		
		err = -EINVAL;
		goto out;
	}
	...
	if (!rpdev) {
		pr_debug("Error tree lookup:vrp_channels %p : vproc_id %x \n",vrp_channels, sa->addr);		
		err = -EINVAL;
		goto out;
	}
