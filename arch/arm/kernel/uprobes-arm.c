...
static int uprobes_substitute_pc(unsigned long *pinsn, u32 oregs)
{
	...
	insn = __mem_to_opcode_arm(*pinsn);
	temp = insn;
	regs = oregs;
	mask = 0xf;
...
enum probes_insn
decode_pc_ro(probes_opcode_t insn, struct arch_specific_insn *asi,
...
enum probes_insn
decode_ldr(probes_opcode_t insn, struct arch_specific_insn *asi,
	   struct decode_header *d)
{
