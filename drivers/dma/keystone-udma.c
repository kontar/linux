...
static int udma_user_get_id(struct udma_user *user, void *handle)
{
	int error, id;

retry:
	if (unlikely(!idr_pre_get(&user->idr, GFP_KERNEL))) {
		dev_err(udma_user_dev(user), "failed to get chan id\n");
		return -EAGAIN;
	}

	spin_lock(&udma_lock);
	error = idr_get_new(&user->idr, handle, &id);
	spin_unlock(&udma_lock);
...
static void udma_user_put_id(struct udma_user *user, int id)
{
	idr_remove(&user->idr, id);
}
...
static int udma_chan_setup_dma(struct udma_chan *chan)
{
	...
	if (error < 0) {
		dev_err(dev, "(%s) failed to set direction\n",
			udma_chan_name(chan));
...
static void udma_chan_destroy(struct udma_chan *chan)
{
	...
	udma_user_put_id(chan->user, chan->id);
	kfree(chan->req);
	dev_dbg(udma_chan_dev(chan), "(%s) chan destroyed: id %d\n",
		udma_chan_name(chan), id);
	kfree(chan);
}
...
static void udma_chan_complete_cb(void *data)
{
...
static int udma_fop_mmap(struct file *file, struct vm_area_struct *vma)
{
...
static long udma_fop_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg)
{
	struct udma_user *user = file->private_data;
...
static int udma_fop_release(struct inode *inode, struct file *file)
{
	struct udma_user *user = file->private_data;
	struct udma_device *udma = user->udma;
	struct udma_map *map;
...
static const struct file_operations udma_fops = {
	.owner          = THIS_MODULE,
	.open           = udma_fop_open,
	.release        = udma_fop_release,
	.mmap		= udma_fop_mmap,
	.unlocked_ioctl = udma_fop_ioctl,
};
