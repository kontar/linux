...
#ifdef DDR_DEBUG
static int msmc_mem_used = 0x13800000;
static int msmc_mem_used2 = 0x0c1a0000;
#endif
...
#ifdef DDR_DEBUG
static void memcheck(struct keystone_dma_chan *chan,
		     const char *where, int offset, const char *base,
		     char val, int len)
{
	int i, corrupt_start = -1, corrupt_end = -1;

	for (i = 0; i < len; i++) {
		if (base[i] != val) {
			corrupt_start = offset + i;
			break;
		}
	}
	if (corrupt_start == -1)
	...
	for (i = len - 1; i >= 0; i--) {
		if (base[i] != val) {
			corrupt_end = offset + i;
			break;
		}
	}
	dev_err(chan_dev(chan), "data corruption at %s, bytes %d-%d"
				"expected %d found %d\n",
		where, corrupt_start, corrupt_end, val, base[i]);
			 ...
			 enum dma_status status, int budget, bool in_poll)
{
	...
	while (budget < 0 || packets < budget) {
		...
		do {
			...
			memcheck(chan, "end", 16 + sg->length, base + 16 + sg->length,
				 0x17, (SZ_2K - 16 - sg->length));
		} while (0);
#endif
...
static void chan_stop(struct keystone_dma_chan *chan)
{
	...
	chan_complete(chan, chan->q_submit, DMA_ERROR, -1, false);
...
static int chan_setup_descs(struct keystone_dma_chan *chan)
{
	...
	if (chan->num_descs > 64) {
		...
		chan->dma_start = msmc_mem_used;
		msmc_mem_used += (SZ_2K * chan->num_descs);

		dev_info(chan_dev(chan), "Channel virt Address = 0x%x\n",
			 (u32)chan->virt_start);
		dev_info(chan_dev(chan), "Channel dma Address = 0x%x\n",
			 chan->dma_start);
		dev_info(chan_dev(chan), "Channel MSMC memory used = 0x%x\n",
			 msmc_mem_used);

	} else {
		chan->virt_start = ioremap(msmc_mem_used2, SZ_2K * chan->num_descs);
		chan->dma_start = msmc_mem_used2;
		msmc_mem_used2 += (SZ_2K * chan->num_descs);
		   ...
		   unsigned long options)
{
	...
	do {
		...
		memset(base + 16 + sg->length, 0x17, (SZ_2K - 16 - sg->length));
	} while (0);
#endif
