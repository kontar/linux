...
#define PASAHO_SET_CMDID(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 29,3)
...
#define PASAHO_SINFO_SET_PAYLOAD_OFFSET(x, v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 16, 8)

/* Set the payload length */
#define PASAHO_SINFO_SET_PAYLOAD_LENGTH(x, v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 0,  16)
 ...
 */
#define PASAHO_SET_N(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 28, 1)

/*
 * Sets the E bit which indicates the extened
 * parameters (packet type) are present for SRIO
 */
#define PASAHO_SET_E(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 27, 1)

/*
 * Sets the destination of the route defined */
#define PASAHO_SET_DEST(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 24, 3)

/* Specifies the flow to use for packets sent to the host */
#define PASAHO_SET_FLOW(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 16, 8)

/* Specifies the queue to use for packets send to the host */
#define PASAHO_SET_QUEUE(x,v)   ((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 0,  16)

/* Specifies the packet type to use for packets send to the SRIO */
#define PASAHO_SET_PKTTYPE(x,v) ((x)->word1) = PASAHO_SET_BITFIELD((x)->word1, (v), 24, 8)
 ...
 */
#define PASAHO_CHKCRC_SET_NEG0(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 23, 1)

/* Sets the optional flags of the CRC/Checksum command */
#define PASAHO_CHKCRC_SET_CTRL(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 16, 8)

/* Sets the start offset of the checksum/crc */
#define PASAHO_CHKCRC_SET_START(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 0,  16)

/* Sets the length of the checksum/crc */
#define PASAHO_CHKCRC_SET_LEN(x,v)	((x)->word1) = PASAHO_SET_BITFIELD((x)->word1, (v), 16, 16)

/* Sets the offset to where to paste the checksum/crc into the packet */
#define PASAHO_CHKCRC_SET_RESULT_OFF(x,v)	((x)->word1) = PASAHO_SET_BITFIELD((x)->word1, (v), 0,  16)

/* Sets the initial value of the checksum/crc */
#define PASAHO_CHKCRC_SET_INITVAL(x,v)	((x)->word2) = PASAHO_SET_BITFIELD((x)->word2, (v), 16, 16)
 ...
 */                         
#define PASAHO_BPATCH_SET_OFFSET(x,v)		((x)->word0) = \
...
#define PASAHO_SET_REPORT_FLOW(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 16, 8)

/* Specifies the queue to use for report packets send to the host */
#define PASAHO_SET_REPORT_QUEUE(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 0,  16)
...
#define PASAHO_SET_SUB_CODE_IP_FRAG(x) ((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, PASAHO_SUB_CMD_IP_FRAG, 24, 5)

/* Specifies the sub-command code */
#define PASAHO_SET_SUB_CODE(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 24, 5)

/* Specifies the offset to the IP header to be fragmented */
#define PASAHO_SET_IP_OFFSET(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 16, 8)

/* Specifies the MTU size */
#define PASAHO_SET_MTU_SIZE(x,v)	((x)->word0) = PASAHO_SET_BITFIELD((x)->word0, (v), 0,  16)
