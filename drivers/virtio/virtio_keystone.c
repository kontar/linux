...
static int __devinit kvdev_add_vq_node(struct keystone_vdev *kvdev,
...
static __devinit int kvdev_add(struct keystone_virtio *kv,
			       struct device_node *node)
{
	struct keystone_vdev *kvdev;
	struct virtio_device *vdev;
...
static int __devexit kvdev_remove(struct keystone_vdev *kvdev)
{
...
dma_get_regs(struct keystone_virtio *kv, int index, const char *name,
	     unsigned *_size)
{
...
static int __devinit kvirtio_probe(struct platform_device *pdev)
{
	...
	kv->reg_global = dma_get_regs(kv, 0, "global", &size);
	if (!kv->reg_global)
		return -ENODEV;
	...
	kv->reg_tx_chan = dma_get_regs(kv, 1, "txchan", &size);
	if (!kv->reg_tx_chan)
		return -ENODEV;
	kv->max_tx_channel = size / sizeof(struct reg_chan);

	kv->reg_rx_chan = dma_get_regs(kv, 2, "rxchan", &size);
	if (!kv->reg_rx_chan)
		return -ENODEV;
	kv->max_rx_channel = size / sizeof(struct reg_chan);

	kv->reg_tx_sched = dma_get_regs(kv, 3, "txsched", &size);
	if (!kv->reg_tx_sched)
		return -ENODEV;
	...
	kv->reg_rx_flow = dma_get_regs(kv, 4, "rxflow", &size);
	if (!kv->reg_rx_flow)
		return -ENODEV;
...
static int __devexit kvirtio_remove(struct platform_device *pdev)
{
...
static struct platform_driver kvirtio_driver = {
	.probe	= kvirtio_probe,
	.remove	= __devexit_p(kvirtio_remove),
	.driver = {
