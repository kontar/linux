					...
					struct keystone_pcie_info *info)
{
	...
	info->reg_cfg_virt = (u32) ioremap_nocache(info->cfg_resource.start,
...
static int get_and_clear_err(u32 reg_virt)
{
 ...
 */
static inline void set_dbi_mode(u32 reg_virt)
{
	u32 val;
 ...
 * status back to ensure the transition is complete.
 */
static inline void clear_dbi_mode(u32 reg_virt)
{
	u32 val;
...
static void disable_bars(u32 reg_virt)
{
...
static void set_outbound_trans(struct keystone_pcie_info *info)
{
	...
	u32 reg_virt = info->reg_cfg_virt;
	int i, tr_size;
...
static void mask_legacy_irq(u32 reg_virt, int i)
{
...
static void unmask_legacy_irq(u32 reg_virt, int i)
{
	unsigned int offset = i << 4;
 ...
 */
static int check_device(u32 reg_virt, struct pci_bus *bus, int dev)
{
 ...
 * single root port.
 */
static inline u32 setup_config_addr(u32 reg_virt, u8 bus, u8 device,
					 u8 function)
{
	u32 addr, regval;
