...
static int __devinit keystone_ipc_gpio_probe(struct platform_device *pdev)
{
...
static int __devexit keystone_ipc_gpio_remove(struct platform_device *pdev)
{
	struct gpio_bank *bank = platform_get_drvdata(pdev);
	int error;

	error = bank ? gpiochip_remove(&bank->chip) : 0;
	if (!error)
