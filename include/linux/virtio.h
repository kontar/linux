...
/**
 * virtqueue_get_queue_index - return the index of a given vq
 * @vq: the struct virtqueue we're talking about.
 */
static inline int virtqueue_get_queue_index(struct virtqueue *vq)
{
	if (vq && vq->ops && vq->ops->get_queue_index)
		return vq->ops->get_queue_index(vq);
	return -ENOTSUPP;
}
